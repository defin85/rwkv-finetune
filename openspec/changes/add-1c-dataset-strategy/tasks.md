...
- [ ] 2.2 Реализовать нормализацию в канонический sample-контракт (`user_prompt`/`assistant_response`) с обязательной проверкой: `user_prompt` на русском языке.
...
- [ ] 4.2 Реализовать профильный formatter/export adapter для релизного train-артефакта (например, `Instruction/Response + <|endoftext|>` для `1C-Expert-v4`) и выпуск `manifest` как артефакта версии.
...
- [ ] 5.1 Провести smoke-проход: `prepare_binidx.sh` на релизном train-артефакте выбранного профиля и проверить валидность `data_prefix`.
